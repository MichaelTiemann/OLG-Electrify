function F=Electrify_4HouseholdReturnFn_single( ...
...
if aprime<0 && agej*ypp<11
    maxloan=-0.5*((single(10)+ypp)-agej*ypp)/10;
...
if c_pp<=0 && saprime<1 && cprime==0 && hprime==0 && labor>0
    ...
    participation_pp=0.02*(single_1+labor)*ypp;
    benefit_pp=-c_pp;
...
if c_pp>0
    ...
    F=single_1+(((c_pp^(single_1-sigma_h-sigma_c))*(hs^sigma_h)*(carservices^sigma_c))^(single_1-sigma))/(single_1-sigma) -psi*(labor^(single_1+eta))/(single_1+eta); % The utility function
...
if agej==J % Final period
    if net_worth_prime<0
        ...
        warmglow=warmglow1*(net_worth_prime^(single_1-warmglow2))/(single_1-warmglow2);
        F=F+warmglow;
    end
end
