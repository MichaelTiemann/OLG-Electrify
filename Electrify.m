...
Params.r=0.05;
Params.r_pp=(1+Params.r)^Params.ypp-1;
...
% Asset returns
Params.r_wedge=0.05; % Rate difference between deposits and loans
...
Params.n_pp=(1+0.02)^Params.ypp-1; % percentage rate (expressed as fraction) of population growth per period
...
for KK=4:9:n_a.firm-9
    this_D=Params.D_pp^(1/Params.ypp);
    this_F=Electrify_FirmReturnFn( ...
        this_D,a_grid.firm(KK+9),a_grid.firm(KK),z_grid.firm(round(n_z.firm/2)), ...
        Params.w,Params.D_pp, ...
        Params.ypp,Params.delta,Params.alpha_k,Params.alpha_l,Params.capadjconstant,Params.tau_corp,Params.phi,Params.tau_d,Params.tau_cg);
    if isinf(this_F)
        ...
        fprintf("Electrify_FirmReturnFn(dividend=%.2f,kprime=%.2f,k=%.2f,z(%d)=%.2f,Params.w=%.2f => %f \n", ...
            this_D, a_grid.firm(KK+9), a_grid.firm(KK), round(n_z.firm/2), z_grid.firm(round(n_z.firm/2)), Params.w, this_F);
    end
end
...
if Params.scenario<3
    GEPriceParamNames={'pension','AccidentBeqS_pp','G','w','firmbeta','D','P0'};
else
    GEPriceParamNames={'pension','AccidentBeqS_pp','AccidentBeqAH_pp','G','w','firmbeta','D','P0'};
end
heteroagentoptions.constrainpositive={'w','P0'};

% We don't need P
% We can get P from the equation that defines r as the return to the mutual fund
% 1+r_pp = (P0 +(1-tau_d)((1+D)^ypp-1) - tau_cg(P0-P))/Plag
% We are looking at stationary general eqm, so
% Plag=P;
% And thus we have
% P=((1-tau_cg)*P0 + (1-tau_d)*((1+D)^ypp-1))/(1+r_pp-tau_cg);
...
if Params.scenario<3
    ...
    FnsToEvaluate.CapitalGainsTaxRevenue.household = @(labor,sprime,s,z,e,ypp,tau_cg,P0,D,tau_d,r_pp) ...
        tau_cg*(P0-(((1-tau_cg)*P0 + (1-tau_d)*((1+D)^ypp-1))/(1+r_pp-tau_cg)))*s; % tau_cg*(P0-Plag)*s, but substitute P=Plag, and then substitute for P
else
    ...
    FnsToEvaluate.CapitalGainsTaxRevenue.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,ypp,tau_cg,P0,D,tau_d,r) ...
        tau_cg*(P0-(((1-tau_cg)*P0 + (1-tau_d)*((1+D)^ypp-1))/(1+r_pp-tau_cg)))*s+(1-tau_d)*r_pp*max(a,0); % tau_cg*(P0-Plag)*s + deposit interest, but substitute P=Plag, and then substitute for P
end
...
GeneralEqmEqns.govbudget = @(G,tau_d,D,ypp,CapitalGainsTaxRevenue,CorpTaxRevenue) G-tau_d*((1+D)^ypp-1)-CapitalGainsTaxRevenue-CorpTaxRevenue; % G is equal to the target, GdivYtarget*Y
...
GeneralEqmEqns.dividends = @(D,DividendPaid_pp,ypp) ((1+D)^ypp-1)-DividendPaid_pp; % That the dividend households receive equals that which firms give
GeneralEqmEqns.ShareIssuance = @(Sissued,P0,D,tau_cg,tau_d,r_pp,ypp) ...
    P0-((((1-tau_cg)*P0 + (1-tau_d)*((1+D)^ypp-1))/(1+r_pp-tau_cg))-Sissued); % P0=P-S, but substitute for P (see derivation inside the return fn)
...
if Params.scenario<3
    FnsToEvaluate.Consumption.household=@( ...
            labor,sprime,s,z,e, ...
            r,pension,AccidentBeqS_pp,w,P0,D,Lhscale, ...
            ...
            labor,0,sprime,0,0,s,0,0,0,z,e, ...
            r,pension,AccidentBeqS_pp,0,w,P0,D,Lhscale, ...
            kappa_j,tau_l,tau_d,tau_cg,ypp,agej,Jr,...
            r_wedge,f_htc,rentprice,agej_pct_cost,pv_pct_cost,energy_pct_cost);
    FnsToEvaluate.Income.household=@( ...
            labor,sprime,s,z,e, ...
            r,pension,AccidentBeqS_pp,AccidentBeqAH,w,P0,D,Lhscale, ...
            kappa_j,tau_l,tau_d,tau_cg,ypp,agej,Jr, ...
    ...
    FnsToEvaluate.Consumption.household=@( ...
            labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e, ...
            r,pension,AccidentBeqS_pp,AccidentBeqAH_pp,w,P0,D,Lhscale, ...
            kappa_j,tau_l,tau_d,tau_cg,ypp,agej,Jr,...
            r_wedge,f_htc,rentprice,agej_pct_cost,pv_pct_cost,energy_pct_cost ...
        ) Electrify_HouseholdConsumptionFn( ...
            labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e, ...
