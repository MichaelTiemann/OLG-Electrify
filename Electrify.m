...
houseservices=[1,1,0.5]; % housing services as a fraction of house value
...
% Asset returns
Params.r_wedge=(1+0.05)^Params.ypp-1; % Rate difference between deposits and loans
...
Params.n=(1+0.02)^Params.ypp-1; % percentage rate (expressed as fraction) of population growth per period
...
Params.delta=(1+0.05)^Params.ypp-1; % Depreciation of physical capital
...
Params.w=1;
AccidentBeqS= [0.01,0.01,0.01]; % Accidental bequests (this is the lump sum transfer of shares)
AccidentBeqAH= [0,0,0.01]; % Accidental bequests (this is the lump sum transfer of assets+house value)
Params.AccidentBeqS=AccidentBeqS(Params.scenario);
Params.AccidentBeqAH=AccidentBeqAH(Params.scenario);
...
if Params.scenario<3
    ...
    this_F=Electrify_HouseholdReturnFn( ...
        ...
        Params.pension,Params.AccidentBeqS,Params.AccidentBeqAH,Params.w,Params.P0,Params.D, ...
        Params.sigma,Params.psi,Params.eta,Params.sigma_h,Params.kappa_j(this_J),Params.tau_l,Params.tau_d,Params.tau_cg,Params.warmglow1,Params.warmglow2,this_J,Params.Jr,Params.J,Params.Lhscale, ...
        Params.scenario,Params.ypp,Params.r,Params.r_wedge,Params.f_htc,1,Params.rentprice,Params.f_coll,Params.houseservices,Params.agej_pct_cost(this_J),Params.pv_pct_cost,Params.energy_pct_cost);
    ...
    this_F=Electrify_HouseholdReturnFn( ...
        ...
        Params.pension,Params.AccidentBeqS,Params.AccidentBeqAH,Params.w,Params.P0,Params.D, ...
        Params.sigma,Params.psi,Params.eta,Params.sigma_h,Params.kappa_j(this_J),Params.tau_l,Params.tau_d,Params.tau_cg,Params.warmglow1,Params.warmglow2,this_J,Params.Jr,Params.J,Params.Lhscale, ...
        Params.scenario,Params.ypp,Params.r,Params.r_wedge,Params.f_htc,Params.minhouse,Params.rentprice,Params.f_coll,Params.houseservices,Params.agej_pct_cost(this_J),Params.pv_pct_cost,Params.energy_pct_cost);
...
for KK=4:9:n_a.firm-9
    this_F=Electrify_FirmReturnFn( ...
        ...
        Params.w, ...
        Params.D,Params.delta,Params.alpha_k,Params.alpha_l,Params.capadjconstant,Params.tau_corp,Params.phi,Params.tau_d,Params.tau_cg);
    if isinf(this_F)
...
ReturnFn.firm=@( ...
        d,kprime,k,z, ...
        w, ...
        D,delta,alpha_k,alpha_l,capadjconstant,tau_corp,phi,tau_d,tau_cg ...
...
if Params.scenario<3
    GEPriceParamNames={'pension','AccidentBeqS','G','w','firmbeta','D','P0'};
else
...
if Params.scenario<3
    ...
    FnsToEvaluate.PensionSpending.household = @(labor,sprime,s,z,e,pension,agej,Jr) ...
        (agej>=Jr)*pension; % Total spending on pensions
    FnsToEvaluate.PayrollTaxRevenue.household = @(labor,sprime,s,z,e,agej,Jr,tau_l,w,kappa_j,Lhscale) ...
        (agej<Jr)*tau_l*labor*w*kappa_j*exp(z+e)*Lhscale; % Total spending on pensions
    FnsToEvaluate.AccidentalBeqSLeft.household = @(labor,sprime,s,z,e,sj) ...
        sprime*(1-sj); % Accidental share bequests left by people who die
    FnsToEvaluate.CapitalGainsTaxRevenue.household = @(labor,sprime,s,z,e,tau_cg,P0,D,tau_d,r) ...
    ...
    FnsToEvaluate.PensionSpending.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,pension,agej,Jr) ...
        (agej>=Jr)*pension; % Total spending on pensions
    FnsToEvaluate.PayrollTaxRevenue.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,agej,Jr,tau_l,w,kappa_j,Lhscale) ...
        (agej<Jr)*tau_l*labor*w*kappa_j*exp(z+e)*Lhscale; % Total spending on pensions
    FnsToEvaluate.AccidentalBeqSLeft.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,sj) ...
        sprime*(1-sj); % Accidental share bequests left by people who die
    % AccidentalBeqAHLeft is zero (if in debt) or accidental asset+house bequests left by people who die
    FnsToEvaluate.AccidentalBeqAHLeft.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,scenario,sj,agej_pct_cost) ...
    ...
    FnsToEvaluate.BadDebt.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,scenario,sj,agej_pct_cost) ...
        min(0,(aprime+(1+agej_pct_cost)*hprime)*(1-sj));
    FnsToEvaluate.CapitalGainsTaxRevenue.household = @(labor,buyhouse,sprime,aprime,hprime,s,a,h,solarpv,z,e,tau_cg,P0,D,tau_d,r) ...
        tau_cg*(P0-(((1-tau_cg)*P0 + (1-tau_d)*D)/(1+r-tau_cg)))*s+(1-tau_d)*r*max(a,0); % tau_cg*(P0-Plag)*s + deposit interest, but substitute P=Plag, and then substitute for P
end

% From firms
FnsToEvaluate.Output.firm = @(d,kprime,k,z,w,alpha_k,alpha_l) ...
...
FnsToEvaluate.Sissued.firm = @(d,kprime,k,z,w,D,delta,alpha_k,alpha_l,capadjconstant,tau_corp,phi) ...
    Electrify_FirmShareIssuance(d,kprime,k,z,w,D,delta,alpha_k,alpha_l,capadjconstant,tau_corp,phi); % Share issuance
FnsToEvaluate.CorpTaxRevenue.firm = @(d,kprime,k,z,w,D,delta,alpha_k,alpha_l,capadjconstant,tau_corp,phi) ...
    Electrify_FirmCorporateTaxRevenue(d,kprime,k,z,w,D,delta,alpha_k,alpha_l,capadjconstant,tau_corp,phi); % revenue from the corporate profits tax
...
if Params.scenario<3
    FnsToEvaluate.Consumption.household=@( ...
            ...
            labor,0,sprime,0,0,s,0,0,0,z,e, ...
            r,pension,AccidentBeqS,0,w,P0,D,Lhscale, ...
            kappa_j,tau_l,tau_d,tau_cg,agej,Jr,...
            r_wedge,f_htc,rentprice,agej_pct_cost,pv_pct_cost,energy_pct_cost);
